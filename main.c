...
void showAlphabet (int pos);



int main(void){
  int ms=0, s=0, key, pKey=-1, c=0, idle=0, rtc, hh, mm, ss;        // variabler från tidigare paket... Ta bort?
  ...
  int vUsrPos=0;             // mode==0, sorts id för vart användaren är, varje knapptryck gör +- 1 och */ 10. Så slide 0 är 0, slide 1 är 1, slide 2 inforuta är 20. Där varje huvslide hämtar de 3 olika textrutornas info från tilldelad flashadress, detsamma gäller infosliden som hämtar från (en? -- då endast en inforuta) flashadress.
  int rUsrPos=1;             // mode==1, samma här men liten annan konstruktion (se nedan switch) då har flera slides (en för varje ruta i slide). Varje slide hämtar de olika textrutornas info från flash samt har en fjärde rutan per slide (utom slide 0) som är "in/ut rutan" -- till huv. slides tillhörande infoslide. r-mode markerar dock också vilken text-kategori user står på också (varje textruta har ju egen adressrymd).
  int wMode=0;               // tillskrivningsmode
  int wModePos=0;            // wMode==1, "tar ut" view och r moderna mha if else.
  char words[15][18]={"Elias Ahlstrom", "Norway", "+47-58-32-86-44", "Pencillin Allergy", "Blood Type O-", "On Warfarin", "Info 1", "Full Code", "Type 1 Diabetes", "Insulin", "Info 2", "Pacemaker", "Renal Transplant", "TB Active", "Info 3" };      // 0-2 är första slides 3 kategorier, 3-5 är slide 1 kategorier, 6 är slide 1 beskrivningsruta, etc.
  ...
  /* Interrupt */
  // interrupt sker och timer startar. Ser knapptrycket o gör +/- elr switch (från och med andra knapptryck!) på gällande positionsvariabel (v/r).
        ...
        if (wMode){                       // Tillskrivningsmode
          ...
          if (lookUpTbl[key]==11){
            if (wModePos<27){
              ...
              strcpy(words[box], bufLet);             // Slutförning av alfabet tillskrivnings paketet
              for (int i=0; i<17; i++){
                bufLet[i]='\0';
              }
              box=0; letCount=0; wModePos=0; wMode=0;            
              } else {
                char choosen = alphabet[wModePos];    // Tillskrivning av en bokstav
                bufLet[letCount] = choosen;
                letCount++;
              }

            } else if (wModePos==27){                 // Om user skulle trycka "DONE", samma paket återigen.
          ...
          if (lookUpTbl[key]==10){            // A (upp)
            if (vUsrPos==0 && rUsrPos==1){    // logiken för byta mode
              modeSwap--;                     // krävs 5 gåner, tills uppnår modeSwap == ModeSwitch
            }
            if (modeSwap==MODESWITCH){
              if (mode==0){
                mode=1; vUsrPos=0; rUsrPos=1; modeSwap=0;
              } else {
                mode=0; vUsrPos=0; rUsrPos=1; modeSwap=0;
              }
            }
          ...
          if (lookUpTbl[key]==11){      // B (val)
            if (mode==0){               // view mode logiken
              ...
              if (wMode==1) {showAlphabet (wModePos);}
            } else if (mode!=0 && mode!=1){   // om skulle bli tokigt
              vUsrPos=0;
              rUsrPos=0;
              mode=0;
...
/* funktionsdefinitioner Alphabet-mode*/
void showAlphabet (int pos){
  switch (pos){
